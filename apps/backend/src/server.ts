...
export async function buildApp(options: { dbFile?: string; logger?: boolean } = {}): Promise<AppContext> {
  ...
  await app.register(cors, { origin: (origin, callback) => callback(null, !origin || origin === "null" || allowedFrontendOrigins.test(origin)), methods: ["GET", "POST", "PATCH", "DELETE", "OPTIONS"], allowedHeaders: ["content-type", "x-utharness-actor"] });
  const rateLimitMax = Number(process.env.UTHARNESS_RATE_LIMIT ?? 120);
  await app.register(rateLimit, { max: Number.isFinite(rateLimitMax) && rateLimitMax > 0 ? rateLimitMax : 120, timeWindow: "1 minute" });
  ...
  app.get("/api/team-members", async () => ({ members: db.raw.prepare("SELECT * FROM team_members ORDER BY team_id,agent_id").all() }));
  app.get("/api/team-messages", async () => ({ messages: db.raw.prepare("SELECT * FROM mailbox_messages ORDER BY created_at DESC LIMIT 100").all() }));
  ...
  app.get("/api/workflow-runs", async () => ({ runs: db.raw.prepare("SELECT * FROM workflow_runs ORDER BY started_at DESC LIMIT 100").all() }));
  ...
  app.post("/api/terminal", async (request, reply) => { const parsed = z.object({ cwd: z.string().min(1).max(4096), shell: z.string().max(4096).optional(), cols: z.number().int().optional(), rows: z.number().int().optional() }).safeParse(bodyOf(request)); if (!parsed.success) return reply.code(400).send({ error: "Invalid terminal options" }); try { return reply.code(201).send(await terminal.open(parsed.data.cwd, parsed.data.shell, parsed.data.cols, parsed.data.rows, actorOf(request))); } catch (error) { return reply.code(403).send({ error: error instanceof Error ? error.message : "Unable to open terminal" }); } });
